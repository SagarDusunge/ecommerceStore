...
const CartNote: React.FC = () => {
  ...
  const cartClickHandler = (e: React.MouseEvent) => {
    const targetElementId = (e.target as HTMLInputElement).id;
    if (targetElementId === "cart" || targetElementId === "counter_span") {
      dispatch({
        type: "TOGGLE_CART",
        payload: true,
      });
  ...
  return (
    ...
    >
      ...
      <span data-testid="counter_span" id="counter_span">{count}</span>
    </span>
  );
};
